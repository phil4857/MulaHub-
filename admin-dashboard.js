...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const withdrawalBody = document.getElementById("withdrawalsTableBody");
  ...
  async function fetchUsers() {
    if (!tableBody) return showMessage("error", "Users table body missing");
  ...
  window.approveWithdrawal = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchUsers();
        fetchWithdrawals();
      } else showMessage("error", data.detail || "Failed to approve withdrawal");
    } catch (err) {
      console.error(err);
  ...
  window.approveUser = username => adminAction("approve-user", username, `${username} approved`);
  window.resetPassword = username => adminAction("reset-password", username, `Password reset for ${username}`);
  window.terminateUser = username => adminAction("terminate-user", username, `${username} terminated`);
